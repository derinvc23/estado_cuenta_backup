...
class invoice_partner_report(models.TransientModel):
    ...
    def create_excel_header(self,worksheet,main_header_style,header_style,text_left,row):
        worksheet.write_merge(0, 1, 3, 3, 'ESTADO DE CUENTA - CLIENTES', main_header_style)
        row = row
    ...
    def get_invoice(self,partner_id):
        ...
        invoice_ids = self.env['account.invoice'].search(domain, order="date_invoice,number")
        return invoice_ids
...
#                            u_residual += inv.residual
			    cli_total += inv.amount_total
			    cli_residual += inv.residual
                            ...
                            row+=1
                        worksheet.write_merge(row, row, 4, 6, 'TOTAL DE CLIENTE ', right_group_style2)
                        u_total += cli_total
	                worksheet.write(row, 7, cli_total, right_group_style2)
                        u_residual += cli_residual
			worksheet.write(row, 8, cli_residual, right_group_style2)
